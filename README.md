...
остаточные траектории. Sim-to-real обеспечивается тем, что удается избежать вырожденных режимов поведения за счет использования 
ЦГП для формирования структуры походки, а так же благодаря изоляции 
обучения с подкреплением от этого.  
Однако из-за использования заданной структуры движения возникает ограничение системы к адаптивности. Также происходит усложнение архитектуры и процесса обучения, 
что является еще одним ограничением. Следовательно нужен компромисс между физической интерпретируемостью, устойчивостью и универсальностью подхода.  

---

## <a id="problem">  Выявленная проблема </a>

После анализа вышеописанных статей обнаружил, что имеется актуальность проблемы формирования 
устойчивого стояния при использовании обучения с подкреплением для четырёхногих роботов. 
Рассмотренные в статьях подходы в основном ориетированы на задачи ходьбы, 
...
в нейросетевых моделях. Однако удержание равновесия в стойке робота является также актуальным, как и передвижение. Это важно для таких сценариев работы, как: наблюдение объектов, для точного подхода и фиксации перед выполнением дальнейшего действия, либо для планирования последующего маршрута на сложном рельефе.  
В режиме удержания равновесия накопленная ошибка в расчетах, люфт в приводах, неточность контакта с 
...
равновесия.**  
Использование физически согласованных иерархических архитектур управления позволяет преодолеть данное ограничение.
Например, базовый уровень будет обеспечивать устойчивое движение и равновесие, а обучаемый уровень отвечать за адаптацию к
неопределенности среды.  
Похожей является описанная выше статья HiLMa-Res, однако в ней иерархия вводится
как средство повышения надежности и переносимости в реальную систему. В данном
задании внимание уделяем формулировке самой проблемы как структурного
...
В среде MuJoCo с обучением стратегии методом PPO наблюдается следующий эффект: 
...
Значит стандартная функция вознаграждения, ориентированная на 
устойчивость и энергоэффективность, не гарантирует формирования физически 
осмысленного поведения. Следовательно без дополнительных архитектурных, структурных ограничений 
